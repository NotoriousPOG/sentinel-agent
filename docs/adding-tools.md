...
10. A mock, if you need one for `demo_mode`, is constructed only in `build_providers` when the flag is on. Its `provider` starts with `mock:`. Label the payload synthetic. Do not construct it inside an `except` on the live client.
