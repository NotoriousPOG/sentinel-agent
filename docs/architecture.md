...
`SENTINEL_DEMO_MODE` selects mock IP, hash, CVE, and DNS providers when `build_registry` runs, and selects the scripted demo model for `POST /investigations`. Loading settings, `GET /health`, and `POST /alerts` do not call a provider or a model. The flag does not replace a failed live lookup. MITRE still reads the checked-in subset. Local Docker Compose defaults the flag to true so `docker compose up` is an offline demo. With the flag off, missing LLM settings are a configuration error and the investigation is not stored.
