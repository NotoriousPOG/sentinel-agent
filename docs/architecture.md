...
There is no remediation executor, no playbook runner, and no endpoint that applies an action. Approval persists a decision. It does not SSH, call a firewall, or isolate a host. If that capability is ever added, it has to be a separate process that refuses to run unless a stored review has `remediation=approve` for that specific action id. This repository does not contain that process. `examples/floci/runner.py` defines `isolate_instance`, which raises if it is called. The approval step does not call it, and there is no flag that turns it on.
...
`POST /alerts` accepts `{"source": "generic_json" | "wazuh" | "aws_guardduty", "payload": {...}}`, stores the normalized document, and returns it. The same `alert_id` is idempotent: the first write wins, and a later body does not replace it. `GET /alerts/{id}` reloads that row. Validation failures use `missing_field`, `invalid_field`, `invalid_type`, or `unknown_source`, and do not echo the submitted value. A missing id is `alert_not_found`. Windows and Sysmon decoder shapes are not parsed into first-class fields. `aws_guardduty` is the minimal mapper described below, not a second product.

`GuardDutyAdapter` maps one documented GetFindings finding object onto `NormalizedAlert`. It does not call AWS, does not accept an EventBridge envelope, and does not promote addresses from action shapes other than `portProbeAction`. The synthetic fixture and the limits are in `examples/floci/`. That demo is not a rewrite of this pipeline. `CrowdStrikeFalconAdapter`, `DefenderAdapter`, `ElasticAdapter`, and `SplunkAdapter` still raise. They are interfaces with a name you can import, not integrations.
...
`SENTINEL_DEMO_MODE` selects mock IP, hash, CVE, and DNS providers when `build_registry` runs, and selects the scripted demo model for `POST /investigations`. Loading settings, `GET /health`, and `POST /alerts` do not call a provider or a model. The flag does not replace a failed live lookup. MITRE reads the checked-in Enterprise 19.2 extract. Local Docker Compose defaults the flag to true so `docker compose up` is an offline demo. With the flag off, missing LLM settings are a configuration error and the investigation is not stored. `GET /ready` runs `SELECT 1`. Transient provider calls retry up to `provider_max_attempts`. When `SENTINEL_API_KEY` is set, alert, investigation, and metrics routes require that key. `/health` and `/ready` do not.
...
`POST /investigations/{id}/review` persists an `AnalystReview`. Notes are required. Extra fields such as `execute` and `auto_remediate` fail validation. Approving the conclusion is the only path to `COMPLETE`. Rejection records the decision and moves to `FAILED`. It does not return to `INVESTIGATING`. Approving remediation writes the field and calls nothing else. There is no remediation executor. `GET /investigations/{id}/report` returns the stored verified report, or 404 when there is none. At the end of milestone 6, `GET /metrics` was still 501. Milestone 9 replaced that route with process counters.
...
`demo_mode` covers CVE and DNS as well as IP and hash. Compose defaults `SENTINEL_DEMO_MODE` to true so `docker compose up` is an offline path. Fixture rows in `services/providers/fixtures.py` give canned verdicts for the documentation-range addresses and synthetic hashes used by `examples/` and `evals/`. Reliability stays `low` for `mock:` sources, so a fixture `reported_malicious: true` classifies `SUSPICIOUS`, not `MALICIOUS`. Unknown IPs stay null. Unknown domains fail closed. `verify_report` and `weighted_evidence_v1` were not changed. There is still no remediation executor.
